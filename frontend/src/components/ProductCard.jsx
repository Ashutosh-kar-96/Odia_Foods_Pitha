    ...
    <article className="group overflow-hidden rounded-lg border border-temple/10 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-soft">
      ...
      <div className="p-4">
        <div className="flex items-center justify-between gap-3">
          <span className="rounded-full bg-haldi/20 px-3 py-1 text-xs font-bold uppercase tracking-wide text-temple">{product.category}</span>
          <span className="flex items-center gap-1 text-sm font-semibold text-clay">
            <Star size={15} fill="currentColor" /> 4.8
          </span>
        </div>
        <Link to={`/products/${product.slug}`}>
          <h3 className="mt-3 font-display text-xl font-bold text-temple">{product.name}</h3>
        </Link>
        <p className="mt-2 line-clamp-2 min-h-[3rem] text-sm leading-6 text-ink/70">{product.short_description}</p>
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => addToCart(product)} className="rounded-md bg-temple p-3 text-white transition hover:bg-sindoor" title="Add to cart">
              <ShoppingBag size={18} />
            </button>
          </div>
        </div>
